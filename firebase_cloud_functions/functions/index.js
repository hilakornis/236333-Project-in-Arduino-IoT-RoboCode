...
exports.generateCropedImage = functions.storage.object().onFinalize(async(object) => {
    ...
    const thumbFilePath = path.normalize(path.join(fileDir, `${croped_PREFIX}${fileName}`));
    ...
    await spawn('convert', [tempLocalFile, '-geometry', size, '-gravity', 'center', '-crop', crop, tempLocalThumbFile], {
        capture: ['stdout', 'stderr']
    });
    await spawn('convert', [tempLocalFile, '-strip', '-interlace', 'Plane', '-quality', '90', tempLocalFile]);

    console.log('croped image created at', tempLocalThumbFile);
    // Uploading the croped.
    await bucket.upload(tempLocalThumbFile, { destination: thumbFilePath, metadata: metadata });
    console.log('croped uploaded to Storage at', thumbFilePath);
